...
mutual
  wk-cmd-coh : (π : Γ ⊇ Γ₁) (ρ : Δ ⊇ Δ₁) (M : Γ₁ ⊢ Δ₁) -> ⟦ wk-cmd π ρ M ⟧ᶜ ≡ (wkenv π ρ ； ⟦ M ⟧ᶜ)
  wk-cmd-coh π ρ (cut A M K) rewrite wk-tm-coh π ρ M | wk-cotm-coh π ρ K = refl

  wk-val-coh : (π : Γ ⊇ Γ₁) (ρ : Δ ⊇ Δ₁) (V : Γ₁ ⊢ᵛ A ∣ Δ₁) -> ⟦ wk-val π ρ V ⟧ᵛ ≡ (wkenv π ρ ； ⟦ V ⟧ᵛ)
  ...
  wk-tm-coh : (π : Γ ⊇ Γ₁) (ρ : Δ ⊇ Δ₁) (M : Γ₁ ⊢ᵗ A ∣ Δ₁) -> ⟦ wk-tm π ρ M ⟧ᵗ ≡ (wkenv π ρ ； ⟦ M ⟧ᵗ)
  wk-tm-coh π ρ (ret V) rewrite wk-val-coh π ρ V = refl
  wk-tm-coh π ρ (μ M) rewrite wk-cmd-coh π (wk-cong ρ) M =
    funext λ { (γ , k) → funext λ k₂ →
      cong (λ x → ⟦ M ⟧ᶜ (⟦ π ⟧ʷ γ , x)) (funext λ { (inj₁ x) → refl ; (inj₂ y) → refl }) }

  wk-cotm-coh : (π : Γ ⊇ Γ₁) (ρ : Δ ⊇ Δ₁) (K : Γ₁ ∣ A ⊢ᵏ Δ₁) -> ⟦ wk-cotm π ρ K ⟧ᵏ ≡ (wkenv π ρ ； ⟦ K ⟧ᵏ)
  ...
  wk-cotm-coh π ρ (μ̃ M) rewrite wk-cmd-coh (wk-cong π) ρ M = refl
