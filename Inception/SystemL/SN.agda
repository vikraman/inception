...
Redᵛ (X `⇒ Y)  (var i)    = ⊤
Redᵛ (X `⇒ Y) {Γ} {Δ} (lam M) =
  ∀ {Γ₁ Δ₁} (π : Γ₁ ⊇ Γ) (ρ : Δ₁ ⊇ Δ) {W : Γ₁ ⊢ᵛ X ∣ Δ₁} {K : Γ₁ ∣ Y ⊢ᵏ Δ₁}
  → Redᵛ X W → CoRedᵏ Y K → SN (cut Y (letv W (wk-tm (wk-cong π) ρ M)) K)

CoRedᵏ X         (covar i)    = ⊤
CoRedᵏ X {Γ} {Δ} (μ̃ C)       =
  ∀ {Γ₁ Δ₁} (π : Γ₁ ⊇ Γ) (ρ : Δ₁ ⊇ Δ) {V : Γ₁ ⊢ᵛ X ∣ Δ₁}
...
CoRed-wk : (X : Ty) {Γ Δ Γ₁ Δ₁ : Ctx} (π : Γ₁ ⊇ Γ) (ρ : Δ₁ ⊇ Δ) {K : Γ ∣ X ⊢ᵏ Δ}
...
record RedSub {Γ Δ Γ₁ : Ctx} (θ : Sub Γ Δ Γ₁) : Set where
  field red : {X : Ty} (i : Γ₁ ∋ X) → Redᵛ X (sub-mem θ i)
open RedSub
...
RedSub-ext : {Γ Δ Γ₁ : Ctx} {X : Ty} {θ : Sub Γ Δ Γ₁} {V : Γ ⊢ᵛ X ∣ Δ} → RedSub θ → Redᵛ X V → RedSub (sub-ex θ V)
...
Fundamental-cmd : {Γ Δ Γ₁ Δ₁ : Ctx} (θ : Sub Γ Δ Γ₁) (φ : CoSub Γ Δ Δ₁)
                → RedSub θ → CoRedSub φ → (C : Γ₁ ⊢ Δ₁) → SN (sub-cmd θ φ C)
Fundamental-val : {Γ Δ Γ₁ Δ₁ : Ctx} {X : Ty} (θ : Sub Γ Δ Γ₁) (φ : CoSub Γ Δ Δ₁)
                → RedSub θ → CoRedSub φ → (V : Γ₁ ⊢ᵛ X ∣ Δ₁) → Redᵛ X (sub-val θ φ V)
Fundamental-tm  : {Γ Δ Γ₁ Δ₁ : Ctx} {X : Ty} (θ : Sub Γ Δ Γ₁) (φ : CoSub Γ Δ Δ₁)
                → RedSub θ → CoRedSub φ → (M : Γ₁ ⊢ᵗ X ∣ Δ₁) → ∀ {K : Γ ∣ X ⊢ᵏ Δ} → CoRedᵏ X K → SN (cut X (sub-tm θ φ M) K)
Fundamental-cotm : {Γ Δ Γ₁ Δ₁ : Ctx} {X : Ty} (θ : Sub Γ Δ Γ₁) (φ : CoSub Γ Δ Δ₁)
                → RedSub θ → CoRedSub φ → (K : Γ₁ ∣ X ⊢ᵏ Δ₁) → CoRedᵏ X (sub-cotm θ φ K)
