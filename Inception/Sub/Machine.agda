...
data _~>ᵛᵛ_ : VState → VState → Set where
     ...
     ~∙var∷pm∷l~> : (γ : ⟦ Γ ⟧ˣ) → (γ' : ⟦ Γ' ⟧ˣ) → (γ'' : ⟦ Γ'' ⟧ˣ)
                 ...
                 ->    ∙ var i , γ ∷pm⟨ ≡M ⟩ pm M N , γ' ∷l⟨ ≡LHS ⟩ tail
                      ~>ᵛᵛ
     ...
     ~∙var∷pm∷r~> : (γ : ⟦ Γ ⟧ˣ) → (γ' : ⟦ Γ' ⟧ˣ) → (γ'' : ⟦ Γ'' ⟧ˣ)
                 ...
                 → (tail : valStack (pair LHS RHS) γ'')
                 ->    ∙ var i , γ ∷pm⟨ ≡M ⟩ pm M N , γ' ∷r⟨ ≡RHS ⟩ tail
                      ~>ᵛᵛ
...
{-
-- We probably can't use this to reduce the number of constructors for _~>ᵛᵛ_ :(
pair∷pm-left :   (γ : ⟦ Γ ⟧ˣ) → (γ' : ⟦ Γ' ⟧ˣ)
      → (x : Γ ⊢ᵛ X)
      → (y : Γ ⊢ᵛ Y)
      → (M : Γ' ⊢ᵛ X `× Y)
      → (N : (Γ' ∙ X ∙ Y) ⊢ᵛ Z')
...
pair∷pm-left γ γ' x y M N ≡M (.(pm M N) , .γ' ■) = pair x y , γ ∷pm⟨ ≡M ⟩ pm M N , γ' ■
pair∷pm-left γ γ' x y M N ≡M ((pm M N) , .γ' ∷pm⟨ M≡M' ⟩ tail) = pair x y , γ ∷pm⟨ ≡M ⟩ pm M N , γ' ∷pm⟨ M≡M' ⟩ tail
pair∷pm-left γ γ' x y M N ≡M ((pm M N) , .γ' ∷l⟨ L≡L' ⟩ tail) =  pair x y , γ ∷pm⟨ ≡M ⟩ pm M N , γ' ∷l⟨ L≡L' ⟩ tail
pair∷pm-left γ γ' x y M N ≡M (.(pm M N) , .γ' ∷r⟨ R≡R' ⟩ tail) =  pair x y , γ ∷pm⟨ ≡M ⟩ pm M N , γ' ∷r⟨ R≡R' ⟩ tail
-}
