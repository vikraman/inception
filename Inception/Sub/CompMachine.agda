...
module CMain {R₀ : Ty} (k₀ : ⟦ R₀ ⟧ → R) where
  ...
  mutual
    ...
    app-eval (var i) N γ π cs πₓ n with lookup (wk-mem π i) γ
    ...
    comp-eval (push W V) γ π cs πₓ n with comp-eval W γ π ((wk-comp (wk-cong π) V) ⊲ γ ⦂⦂ cs) wk-id n
    ...
    comp-eval (var {A = X} M) γ π cs πₓ n with val-eval-rec {X = `V} M γ π
