...
module EnvMain {R₀ : Ty} (k₀ : ⟦ R₀ ⟧ → R) where
  ...
  mem-gc-helper {Γ = Γ Cx.∙ X} Cx.h (wk-cong π) = cong (λ x → t (wk-mem x h)) wk-trans-id'
  mem-gc-helper {Γ = Γ Cx.∙ X} Cx.h (wk-wk π) =
      t (wk-mem (wk-trans π (wk-wk wk-id)) h)
    ≡⟨ cong t (mem-gc-helper h π) ⟩
      t (wk-mem π (t h)) ∎
  mem-gc-helper {Γ = Γ Cx.∙ X} (Cx.t i) (wk-cong π) = cong (λ x → t (wk-mem x (t i))) wk-trans-id'
  mem-gc-helper {Γ = Γ Cx.∙ X} (Cx.t i) (wk-wk π) = cong t (mem-gc-helper (t i) π)

  mem-gc : (i : Γ ∋ X) → MemGC i
  mem-gc {Γ = Γ ∙ X} h = ε ∙ X , wk-cong wk-wk-ε , h ,
                        λ π' →
                            wk-mem π' h
                          ≡⟨ refl ⟩
                            wk-mem π' (wk-mem (wk-cong wk-wk-ε) h)
                          ≡⟨ wk-mem-trans h π' (wk-cong wk-wk-ε) ⟩
                            wk-mem (wk-trans π' (wk-cong wk-wk-ε)) h ∎
  mem-gc (t i) =
      let
        IH = mem-gc i
        c = (proj₂ (proj₂ (proj₂ IH)))
      in
      proj₁ IH , wk-wk (proj₁ (proj₂ IH)) , proj₁ (proj₂ (proj₂ IH)) ,
      λ π' →
          let
            c' = c (wk-trans π' (wk-wk wk-id))
            c'' : wk-mem π' (t i) ≡ wk-mem (wk-trans π' (wk-wk (proj₁ (proj₂ IH)))) (proj₁ (proj₂ (proj₂ IH)))
            c'' =   wk-mem π' (t i)
                  ≡⟨ sym (wk-wk-trans-id π' i) ⟩
                    wk-mem (wk-trans π' (wk-wk wk-id)) i
                  ≡⟨ c' ⟩
                    wk-mem (wk-trans (wk-trans π' (wk-wk wk-id)) (proj₁ (proj₂ IH))) (proj₁ (proj₂ (proj₂ IH)))
                  ≡⟨ cong (λ x → wk-mem x (proj₁ (proj₂ (proj₂ IH)))) (wk-wk-trans π' (proj₁ (proj₂ IH))) ⟩
                    wk-mem (wk-trans π' (wk-wk (proj₁ (proj₂ IH)))) (proj₁ (proj₂ (proj₂ IH))) ∎
          in
          c''

  val-lam-helper : (W : Comp (Γ ∙ X) Y) → CompGC W → ValGC (lam W)
  val-lam-helper {X = X} W (ε , wk-wk π' , W' , eq) =
    let
      a0 : wk-comp (wk-cong {A = X} π') (wk-comp (wk-wk wk-ε) W') ≡ wk-comp (wk-trans (wk-cong π') (wk-wk wk-ε)) W'
      a0 = wk-comp-trans W' (wk-cong π') (wk-wk wk-ε)
    in
    ε , π' , lam (wk-comp (wk-wk wk-id) W') ,
      λ π'' →
        cong lam (  wk-comp (wk-cong π'') W
                  ≡⟨ eq (wk-cong π'') ⟩
                    wk-comp (wk-trans (wk-cong π'') (wk-wk π')) W'
                  ≡⟨ cong (λ x → wk-comp (wk-wk (wk-trans π'' π')) W') a0 ⟩
                    wk-comp (wk-wk (wk-trans π'' π')) W'
                  ≡⟨ cong (λ x → wk-comp x W') (sym wk-trans-id') ⟩
                    wk-comp (wk-trans (wk-cong (wk-trans π'' π')) (wk-wk wk-ε)) W'
                  ≡⟨ sym (wk-comp-trans W' (wk-cong (wk-trans π'' π')) (wk-wk wk-ε)) ⟩
                    wk-comp (wk-cong (wk-trans π'' π')) (wk-comp (wk-wk wk-ε) W') ∎)
  val-lam-helper W (Γ' ∙ X , wk-cong π' , W' , eq) = Γ' , π' , lam W' , λ π'' → cong lam (eq (wk-cong π''))
  val-lam-helper {X = X} W (Γ' ∙ X' , wk-wk π' , W' , eq) =
    let
      a0 : wk-comp (wk-cong {A = X} π') (wk-comp (wk-wk wk-id) W') ≡ wk-comp (wk-trans (wk-cong π') (wk-wk (wk-cong wk-id))) W'
      a0 = wk-comp-trans W' (wk-cong π') (wk-wk wk-id)
      a1 : wk-comp (wk-wk {A = X} π') W' ≡ wk-comp (wk-wk (wk-trans π' (wk-cong wk-id))) W'
      a1 = cong (λ x → wk-comp x W') (sym (cong wk-wk wk-trans-id'))
    in
    Γ' ∙ X' , π' , lam (wk-comp (wk-wk wk-id) W') ,
      λ π'' →
        cong lam (  wk-comp (wk-cong π'') W
                  ≡⟨ eq (wk-cong π'') ⟩
                    wk-comp (wk-trans (wk-cong π'') (wk-wk π')) W'
                  ≡⟨ cong (λ x → wk-comp x W') (sym wk-trans-id') ⟩
                    wk-comp (wk-trans (wk-cong (wk-trans π'' π')) (wk-wk (wk-cong wk-id))) W'
                  ≡⟨ sym (wk-comp-trans W' (wk-cong (wk-trans π'' π')) (wk-wk (wk-cong wk-id))) ⟩
                    wk-comp (wk-cong (wk-trans π'' π')) (wk-comp (wk-wk (wk-cong wk-id)) W') ∎)
  ...
  mutual
    ...
    val-gc (var i) =
      let
        m = (mem-gc i)
      in
      proj₁ m , proj₁ (proj₂ m) , var (proj₁ (proj₂ (proj₂ m))) , λ π' → cong var (proj₂ (proj₂ (proj₂ m)) π')
    val-gc (lam W) = val-lam-helper W (comp-gc W)
    val-gc (pair M₁ M₂) =
      let
        v₁ = val-gc M₁
        M₁' = proj₁ (proj₂ (proj₂ v₁))
        ...
        eq₂' = proj₂ (proj₂ (proj₂ (proj₂ (proj₂ j))))
      in
      Γ' , π , pair (wk-val π₁' M₁') (wk-val π₂' M₂') ,
      λ π' →
        let
          eq₁'' : (wk-val π' M₁) ≡ (wk-val (wk-trans π' π) (wk-val π₁' M₁'))
          eq₁'' =   (wk-val π' M₁)
                  ≡⟨ proj₂ (proj₂ (proj₂ v₁)) π' ⟩
                    wk-val (wk-trans π' π₁) M₁'
                  ≡⟨ cong (λ x → wk-val (wk-trans π' x) M₁') eq₁' ⟩
                    wk-val (wk-trans π' (wk-trans π π₁')) M₁'
                  ≡⟨ cong (λ x → wk-val x M₁') (wk-assoc {π₁ = π'} {π₂ = π} {π₃ = π₁'}) ⟩
                    wk-val (wk-trans (wk-trans π' π) π₁') M₁'
                  ≡⟨ sym (wk-val-trans M₁' (wk-trans π' π) π₁') ⟩
                    (wk-val (wk-trans π' π) (wk-val π₁' M₁')) ∎
          eq₂'' : (wk-val π' M₂) ≡ (wk-val (wk-trans π' π) (wk-val π₂' M₂'))
          eq₂'' =   (wk-val π' M₂)
                  ≡⟨ proj₂ (proj₂ (proj₂ v₂)) π' ⟩
                    wk-val (wk-trans π' π₂) M₂'
        ...
        cong₂ pair eq₁'' eq₂''
    val-gc (pm M N) = {!!}
    val-gc unit = ε , wk-wk-ε , unit , λ {Δ} π' → refl
    ...
    comp-gc (return M) =
      let
        v = val-gc M
      in
      proj₁ v , proj₁ (proj₂ v) , return (proj₁ (proj₂ (proj₂ v))) , λ π' → cong return (proj₂ (proj₂ (proj₂ v)) π')
    comp-gc (pm M W) = {!!}
    comp-gc (push W₁ W₂) = {!!}
    comp-gc (app M N) =
        let
        ...
        λ π' →
          let
            ...
            eq₁'' =   (wk-val π' M)
                  ...
                  ≡⟨ sym (wk-val-trans M' (wk-trans π' π) π₁') ⟩
                    (wk-val (wk-trans π' π) (wk-val π₁' M')) ∎
            eq₂'' : wk-val π' N ≡ wk-val (wk-trans π' π) (wk-val π₂' N')
            eq₂'' =   (wk-val π' N)
                    ≡⟨ proj₂ (proj₂ (proj₂ v₂)) π' ⟩
                      wk-val (wk-trans π' π₂) N'
                    ≡⟨ cong (λ x → wk-val (wk-trans π' x) N') eq₂' ⟩
                      wk-val (wk-trans π' (wk-trans π π₂')) N'
                    ≡⟨ cong (λ x → wk-val x N') (wk-assoc {π₁ = π'} {π₂ = π} {π₃ = π₂'}) ⟩
                      wk-val (wk-trans (wk-trans π' π) π₂') N'
                    ≡⟨ sym (wk-val-trans N' (wk-trans π' π) π₂') ⟩
                      (wk-val (wk-trans π' π) (wk-val π₂' N')) ∎
          in
          cong₂ app eq₁'' eq₂''
    comp-gc (var M) =
      let
  ...
  {-
  mem-gc-proj₁ : (π : Wk Δ Γ) → (i : Γ ∋ X) → proj₁ (proj₁ (mem-gc i)) ≡ proj₁ (proj₁ (mem-gc (wk-mem π i)))
  mem-gc-proj₁ wk-ε ()
  mem-gc-proj₁ (wk-cong π) Cx.h = refl
  mem-gc-proj₁ (wk-cong π) (Cx.t i) = mem-gc-proj₁ π i
  mem-gc-proj₁ (wk-wk π) Cx.h = mem-gc-proj₁ π h
  mem-gc-proj₁ (wk-wk π) (Cx.t i) = mem-gc-proj₁ π (t i)

  --mutual
  --  val-gc-comm : (π : Wk Δ Γ) → (M : Val Γ X) → wk-trans π (proj₁ (proj₂ (val-gc M))) ≡ proj₁ (proj₂ (val-gc (wk-val π M)))
  --  val-gc-comm π M = {!!}

  mutual

    val-gc-proj₁ : (π : Wk Δ Γ) → (M : Val Γ X) → proj₁ (val-gc M) ≡ proj₁ (val-gc (wk-val π M))
    val-gc-proj₁ π (var i) = mem-gc-proj₁ π i
    val-gc-proj₁ π (lam W) = {!!}
    val-gc-proj₁ π (pair M₁ M₂) =
      let
        IH1 = val-gc-proj₁ π M₁
        LHS1 = proj₂ (proj₂ (proj₂ (val-gc M₁)))
        RHS1 = proj₂ (proj₂ (proj₂ (val-gc (wk-val π M₁))))

      in
      {!!}
    val-gc-proj₁ π (pm M N) = {!!}
    val-gc-proj₁ π unit = {!!}

    comp-gc-proj₁ : (π : Wk Δ Γ) → (W : Comp Γ X) → proj₁ (comp-gc W) ≡ proj₁ (comp-gc (wk-comp π W))
    comp-gc-proj₁ π (return M) = val-gc-proj₁ π M
    comp-gc-proj₁ π (pm M W) = {!!}
    comp-gc-proj₁ π (push W₁ W₂) = {!!}
    comp-gc-proj₁ π (app M N) = {!!}
    comp-gc-proj₁ π (var M) = val-gc-proj₁ π M
    comp-gc-proj₁ π (sub W₁ W₂) = {!!}
  -}
